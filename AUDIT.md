...
### 4.5 Security validation is not wired in
`commands/validation.rs` exists with unit tests but is commented out of `mod.rs` re-exports ("Internal use only") and the audit found **no call site** of `validate_path` in the command modules. Path traversal protection is therefore not actually enforced on the IPC boundary.
...
1. Path validation not enforced at IPC boundary → arbitrary file read/write via `fs` commands (any renderer-side XSS escalates to full disk access).
...
1. Enforce `validate_path` (and add args validation) across `fs`, `git`, `search`, `watch`, `tasks`, `debug` command boundaries.

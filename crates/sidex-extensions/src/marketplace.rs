...
impl MarketplaceClient {
    ...
    ///
    /// Retries transient failures (transport errors and 5xx responses) up
    /// to 3 total attempts with exponential backoff; 4xx responses fail
    ...
    pub async fn download_from_url(&self, url: &str) -> Result<Vec<u8>> {
        ...
        for attempt in 1..=MAX_ATTEMPTS {
            ...
            let response = match client.get(url).send().await {
                ...
                Err(err) => {
                    log::warn!("vsix download attempt {attempt}/{MAX_ATTEMPTS} failed: {err}");
            ...
            if status.is_server_error() && attempt < MAX_ATTEMPTS {
                log::warn!(
                    "vsix download attempt {attempt}/{MAX_ATTEMPTS} got HTTP {} — retrying",
                    status.as_u16()
                );
            ...
            match response.bytes().await {
                ...
                Err(err) => {
                    log::warn!(
                        "vsix download attempt {attempt}/{MAX_ATTEMPTS} failed reading body: {err}"
                    );
                    last_error = format!("{err:#}");
                    if attempt < MAX_ATTEMPTS {
                        tokio::time::sleep(backoff).await;
        ...
        Err(anyhow::anyhow!(
            "vsix download failed after {MAX_ATTEMPTS} attempts: {last_error}"
        ))
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn download_from_url_retry_logic_completes() {
        // Validates that the retry loop compiles and terminates without
        // panicking. Real HTTP behaviour is covered by the ignored network
        // smoke tests; here a dead URL only exercises the error path.
        let client = MarketplaceClient::new();
        let result = client
            .download_from_url("https://127.0.0.1:9/nonexistent.vsix")
            .await;
        assert!(result.is_err(), "dead URL must return an error, not hang");
    }
}
